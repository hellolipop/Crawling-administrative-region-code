...
const chalk = require('chalk'); //打印文字颜色
...
function formatCountryCode(code, text = '') {
    // 特殊处理东莞市和中山市的县区数据 code
    if (text === '东莞市' || text === '中山市') {
        return code.slice(0, -3);
    }
                ...
                if (f.includes(pcity.text)) {
                    ...
                    for (let i = 0, l = pareas.length; i < l; i++) {
                        ...
                        if (data.length) {
                            ...
                            data.forEach(item => {
                                if (item.text !== '市辖区') {
                                    pCurAreas[formatCountryCode(item.code)] = item.text.indexOf('办事处') > -1 ? item.text.slice(0, -3) : item.text;
                                    console.log('这是存放的', item);
                                }
                            });
                            pcaa[code] = pCurAreas;
                        }
